...
# if the skip disk flag was set, skip the disk performance test, otherwise test disk performance
	...
	if [[ -z "$PREFER_BIN" && -n "$LOCAL_FIO" ]]; then # local fio has been detected, use instead of pre-compiled binary
		...
		if [[ -n $LOCAL_CURL ]]; then
			curl -s --connect-timeout 5 --retry 5 --retry-delay 0 https://benchs.pika.net.cn/yabs-bench/bin/fio_$ARCH -o "$DISK_PATH/fio"
		else
			wget -q -T 5 -t 5 -w 0 https://benchs.pika.net.cn/yabs-bench/bin/fio_$ARCH -O "$DISK_PATH/fio"
		fi
...
if [ -z "$SKIP_IPERF" ]; then
	...
	if [[ -z "$PREFER_BIN" && -n "$LOCAL_IPERF" ]]; then # local iperf has been detected, use instead of pre-compiled binary
		...
		if [[ -n $LOCAL_CURL ]]; then
			curl -s --connect-timeout 5 --retry 5 --retry-delay 0 https://benchs.pika.net.cn/yabs-bench/bin/iperf3_$ARCH -o "$IPERF_PATH/iperf3"
		else
			wget -q -T 5 -t 5 -w 0 https://benchs.pika.net.cn/yabs-bench/bin/iperf3_$ARCH -O "$IPERF_PATH/iperf3"
		fi
