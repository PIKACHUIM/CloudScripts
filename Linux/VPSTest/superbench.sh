...
benchinit() {
	...
	if  [ ! -e './speedtest-cli/speedtest' ]; then
		...
		wget --no-check-certificate -qO speedtest.tgz https://benchs.pika.net.cn/ecss-bench/superbench/ookla-speedtest-1.2.0-linux-x86_64.tgz > /dev/null 2>&1
	fi
	mkdir -p speedtest-cli && tar zxvf speedtest.tgz -C ./speedtest-cli/ > /dev/null 2>&1 && chmod a+rx ./speedtest-cli/speedtest

	if  [ ! -e 'tools.py' ]; then
		echo " Installing tools.py ..."
		wget --no-check-certificate https://benchs.pika.net.cn/ecss-bench/superbench/tools.py > /dev/null 2>&1
	fi
	chmod a+rx tools.py

	if  [ ! -e 'fast_com.py' ]; then
		echo " Installing Fast.com-cli ..."
		wget --no-check-certificate https://benchs.pika.net.cn/ecss-bench/superbench/fast_com.py > /dev/null 2>&1
		wget --no-check-certificate https://benchs.pika.net.cn/ecss-bench/superbench/fast_com_example_usage.py > /dev/null 2>&1
	fi
