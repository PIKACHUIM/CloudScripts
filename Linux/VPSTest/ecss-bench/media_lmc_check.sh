...
download_extra_data() {
    MEDIA_COOKIE=$(curl ${CURL_OPTS} -s "https://benchs.pika.net.cn/ecss-bench/RegionRestrictionCheck/cookies")
    IATACODE=$(curl ${CURL_OPTS} -s "https://benchs.pika.net.cn/ecss-bench/RegionRestrictionCheck/reference/IATACode.txt")
    IATACODE2=$(curl ${CURL_OPTS} -s "https://benchs.pika.net.cn/ecss-bench/RegionRestrictionCheck/reference/IATACode2.txt")
...
function MediaUnlockTest_InstagramMusic() {
    ...
    if [ "$ARCH" = "x86_64" ]; then
        wget -O ins https://benchs.pika.net.cn/ecss-bench/ins_amd64
        chmod +x ./ins
        clear
        ./ins
    elif [ "$ARCH" = "aarch64" ]; then
        wget -O ins https://benchs.pika.net.cn/ecss-bench/ins_arm64
        chmod +x ./ins
        clear
        ./ins
    else
...
function showGoodbye() {
    ...
    if [ "$LANGUAGE" == 'en' ]; then
        ...
        echo -e ''
        bash <(curl ${CURL_DEFAULT_OPTS} -s https://benchs.pika.net.cn/ecss-bench/RegionRestrictionCheck/reference/AD/ADEN)
    elif [[ "$REGION_ID" == "8" ]]; then
        echo -e "${Font_Green}本次测试已结束，感谢使用此脚本${Font_Suffix}"
        echo -e ''
        echo -e "${Font_Yellow}检测脚本当天运行次数: ${TODAY_RUN_TIMES}; 共计运行次数: ${TOTAL_RUN_TIMES}${Font_Suffix}"
        echo -e ''
        bash <(curl ${CURL_DEFAULT_OPTS} -s https://benchs.pika.net.cn/ecss-bench/RegionRestrictionCheck/reference/AD/ADKR)
        echo -e ''
        ...
        echo -e "${Font_Yellow}检测脚本当天运行次数: ${TODAY_RUN_TIMES}; 共计运行次数: ${TOTAL_RUN_TIMES}${Font_Suffix}"
        echo -e ''
        bash <(curl ${CURL_DEFAULT_OPTS} -s https://benchs.pika.net.cn/ecss-bench/RegionRestrictionCheck/reference/AD/ADJP)
        echo -e ''
    else
        echo -e "${Font_Green}本次测试已结束，感谢使用此脚本${Font_Suffix}"
        echo -e ''
        echo -e "${Font_Yellow}检测脚本当天运行次数: ${TODAY_RUN_TIMES}; 共计运行次数: ${TOTAL_RUN_TIMES}${Font_Suffix}"
        echo -e ''
        bash <(curl ${CURL_DEFAULT_OPTS} -s https://benchs.pika.net.cn/ecss-bench/RegionRestrictionCheck/reference/AD/AD1)
        echo -e ''
        bash <(curl ${CURL_DEFAULT_OPTS} -s https://benchs.pika.net.cn/ecss-bench/RegionRestrictionCheck/reference/AD/AD2)
    fi
}
